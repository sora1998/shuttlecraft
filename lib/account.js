 ...
 * @param bio - The bio parameter is the bio of the actor.
 * @param image - The image parameter is the avatar of the actor.
 * @param canReply - Whether the posts can be replied.
 * @param canBoost - Whether the posts can be boosted.
 * @param canFave - Whether the posts can be liked.
 ...
 */
export const createActor = (name, domain, pubkey, bio, img, canReply, canBoost, canFave) => {
...
export const updateAccount = async (name, domain, bio, img, canReply, canBoost, canFave) => {
  console.log(img);
  // verify domain name
  const re = /^((?:(?:(?:\w[.\-+]?)*)\w)+)((?:(?:(?:\w[.\-+]?){0,62})\w)+)\.(\w{2,6})$/;
  if (!domain.match(re)) {
  ...
  return new Promise((resolve, reject) => {
    ...
    crypto.generateKeyPair(
      ...
      (err, publicKey, privateKey) => {
        ...
        const actorRecord = createActor(name, domain, publicKey, bio, img, canReply, canBoost, canFave);
        const webfingerRecord = createWebfinger(name, domain);
        const apikey = crypto.randomBytes(16).toString('hex');
        const account = {
          actor: actorRecord,
