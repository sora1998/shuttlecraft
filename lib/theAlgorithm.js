...
export const getActivityStream = async (limit, offset) => {
  ...
  for (px = offset; px < sortedSlice.length; px++) {
    const postId = sortedSlice[px];

    // process a post by someone else
    if (postId.type === 'activity') {
      // Ignore posts from people I am not following
      if (!isFollowing(postId.actor)) {
        continue;
      }

      if (!postId.inReplyTo || isReplyToMyPost(postId) || (await isReplyToFollowing(postId))) {
        try {
          const post = await getFullPostDetails(postId.id);
          stream.push(post);
        } catch (err) {
